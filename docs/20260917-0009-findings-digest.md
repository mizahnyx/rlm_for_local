...
|---|---|---|
| D1 | **Ordering-dependent suite failure.** `pytest tests/` → 2 failures in `tests/rlm_kernel/test_corpus.py::TestSearchCoverageIsPublishedNotCounted`; those 4 tests pass in isolation and **`tests/rlm_kernel/` alone is 547 passed / 0 failed**. So it is a *cross-directory* interaction (the class `AGENTS.md` §3 warns about, now affecting the documented invocation). | **Bisect next**: run `tests/` minus the kernel dir plus those two tests; then pairwise (`tests/test_corpus_repl.py` + `tests/rlm_kernel/test_corpus.py`). Do not land anything on top until it is understood. |
